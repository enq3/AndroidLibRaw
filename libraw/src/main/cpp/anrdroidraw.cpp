...
extern "C" JNIEXPORT jint JNICALL Java_com_homesoft_photo_libraw_LibRaw_open(JNIEnv* env, jobject jLibRaw, jstring file, jboolean useThumb){
    const char* nativeString = env->GetStringUTFChars(file, nullptr);
    auto libRaw = getLibRaw(env, jLibRaw);
    int result = libRaw->open_file(nativeString);
    if(result==0) {
        if (useThumb == JNI_TRUE) {
...
extern "C" JNIEXPORT jint JNICALL Java_com_homesoft_photo_libraw_LibRaw_openBufferPtr(JNIEnv* env, jobject jLibRaw, jlong ptr, jint size, jboolean useThumb) {
    auto libRaw = getLibRaw(env, jLibRaw);
...
extern "C" JNIEXPORT jint JNICALL Java_com_homesoft_photo_libraw_LibRaw_openBuffer(JNIEnv* env, jobject jLibRaw, jbyteArray buffer, jint size, jboolean useThumb){
    auto libRaw = getLibRaw(env, jLibRaw);
